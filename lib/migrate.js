...
function runMigrations (direction, migrations, options) {
  ...
  return migrations.reduce(
    (chain, migration) => chain.then(() => migration.code[direction](r, conn)
      .then(emit('info', `Executed migration ${migration.name} ${options.op} (file: ${migration.filename})`))),
    Promise.resolve()
  ).then(() => migrations)
}
...
function saveExecutedMigrationsMetadata (migrations, options) {
  ...
  return migrations
    .map(migration => ({ timestamp: migration.timestamp.toISOString(), name: migration.name, filename: migration.filename, dir: migration.dir }))
