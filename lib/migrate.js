...
function validateOptions (options) {
  const schema = Joi.object().keys({
    ...
    additionalMigrationsDirectory: Joi.array().items(Joi.string())
...
function getMigrationsFromPath (options) {
  const { migrationsDirectory, additionalMigrationsDirectory, relativeTo } = options
  const path = Path.resolve(relativeTo, migrationsDirectory)
  const migrationRegExp = /^(\d{14})-(.*)\.js$/

  let additionalPaths = []
  if (additionalMigrationsDirectory) {
    additionalPaths = additionalMigrationsDirectory.map(directory => Path.resolve(relativeTo, directory))
  }

  return readMigrationFilenamesFromPaths([path, ...additionalPaths])
    .then(files => console.log(...files) || [].concat(...files).filter(file => file.match(migrationRegExp)))
...
function filterMigrationsUntilLimit (migrations, options, direction) {
  if (options.to) {
    const migrationLimit = migrations.find(migration => migration.name === options.to)
    if (!migrationLimit) {
